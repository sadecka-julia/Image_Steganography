from dct import prepareImage, dctTransformation
...
def extractBitsFromImage(image_path, mess_lenght, mode=5):
        ...
        for j in range(width):
            for channel in range(3):
                if channel in [2]:
                    ...
                    # idx = 0
                    # print(f"{dct_blocks[i, j, :, :, channel]}, wyciągamy {bit}")
...
def dctDecoding(stego_path, size):
    bits = extractBitsFromImage(stego_path, size)
    # print(bits)
