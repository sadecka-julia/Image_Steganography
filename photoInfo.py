...
def makeStegoImageDCT(input_folder, output_folder):
    for filename in os.listdir(input_folder):
        if filename.endswith(('.png', '.jpg', '.bmp')):
            ...
            stego_image_name = nameOfTheStegoImage(input_path)
            output_path = os.path.join(output_folder, stego_image_name)
            temp_path = os.path.join(temp_folder, stego_image_name)
...
def cutToEight(input_folder, output_folder):
    for filename in os.listdir(input_folder):
    ...
    for filename in os.listdir(input_folder):
        ...
        if os.path.isfile(file_path) and filename.lower().endswith('.png'):
            try:
                ...
                print(f"Saved image without alpha channel: {output_path}")
...
def canWeReadMessageDCT(stego_folder, worksheet, size_of_mess):
    scores = []
    for filename in os.listdir(stego_folder):
        if filename.endswith(('.png', '.jpg', '.bmp', '.gif', '.jpeg')):
            mess = '--'
            ...
            print("________________", filename, "__________________", img.size)
            try:
                mess = dctDecoding(path, size_of_mess)
                print(mess)
...
def compareImages(before_folder, after_folder, worksheet):
    ...
    for filename in os.listdir(after_folder):
        after_img_folder = os.path.join(after_folder, filename)
        name_af, _ = os.path.splitext(filename)
        # name_af, size_af = imageInfo(after_img)
        after_bytes = os.path.getsize(after_img_folder)
        name_af_full = filename

        before_img_folder = None
        for file in os.listdir(before_folder):
            name_be, _ = os.path.splitext(file)
            if name_be == name_af:
                before_img_folder = os.path.join(before_folder, file)
                name_be_full = file
                break
        
        if before_img_folder:
            try:
                before_bytes = os.path.getsize(before_img_folder)
                print("-------------------------------------------------------")
                _, size_be = imageInfo(before_img_folder)
                _, size_af = imageInfo(after_img_folder)
                cr_value = compression_ratio(before_img_folder, after_img_folder)
                print("CR: ", cr_value)
                mse_value = False
                psnr_value = False
                # mse_value = mse(before_img_folder, after_img_folder)
                # print("MSE: ", mse_value)
                try:
                    mse_value = mse(before_img_folder, after_img_folder)
                    print("MSE: ", mse_value)
                ...
                if mse_value:
                    if mse_value == 0.0:
                        ...
                        try:
                            psnr_value = psnr(before_img_folder, after_img_folder)
        ...
        score = [name_be_full, name_af_full, size_be, size_af, before_bytes, after_bytes, cr_value, mse_value, psnr_value]
        scores.append(score)
    
    row = 1
    col = 1
    worksheet.write(0, 1, "Nazwa przed")
    worksheet.write(0, 2, "Nazwa po")
    worksheet.write(0, 3, "Rozmiar przed")
    worksheet.write(0, 4, "Rozmiar po")
    worksheet.write(0, 5, "KB przed")
    worksheet.write(0, 6, "KB po")
    worksheet.write(0, 7, "CR")
    worksheet.write(0, 8, "MSE")
    worksheet.write(0, 9, "PSNR")

    # Zapis wyników do tabeli
    for name1, name2, size1, size2, bytes1, bytes2, vcr, vmse, vpsnr in scores:
        worksheet.write(row, col, name1)
        worksheet.write(row, col+1, name2)
        worksheet.write(row, col+2, str(size1))
        worksheet.write(row, col+3, str(size2))
        worksheet.write(row, col+4, bytes1//1000)
        worksheet.write(row, col+5, bytes2//1000)
        worksheet.write(row, col+6, vcr)
        worksheet.write(row, col+7, vmse)
        worksheet.write(row, col+8, vpsnr)
        row += 1
...
if __name__ == '__main__':
    workbook = xlsxwriter.Workbook('D:\STUDIA\Cyberka\Inzynierka\Wyniki_excel\Steghide.xlsx')    
    worksheet = workbook.add_worksheet("LSB")
    ...
    after_folder = 'D:\STUDIA\Cyberka\Inzynierka\Zbiory\Signal_lsb_przeslane'
    dct_folder = "D:\STUDIA\Cyberka\Inzynierka\Zbiory\Zbior_DCT_mode3v3"
    mini_dct_folder = 'D:\STUDIA\Cyberka\Inzynierka\Zbiory\Mini_DCT'
    mini_folder = 'D:\STUDIA\Cyberka\Inzynierka\Zbiory\Mini_zbior'
    app_folder = "D:\STUDIA\Cyberka\Inzynierka\Zbiory\App"
    signal_folder = 'D:\STUDIA\Cyberka\Inzynierka\Zbiory\Signal_lsb_przeslane_pobrane'
    lsb_folder = "D:\STUDIA\Cyberka\Inzynierka\Zbiory\Zbior_LSB"
    eight_folder = "D:\STUDIA\Cyberka\Inzynierka\Zbiory\Zbior_8_dlaDCT"
    temp_folder = "D:\STUDIA\Cyberka\Inzynierka\Zbiory\Temp"
    pop_dct = "D:\STUDIA\Cyberka\Inzynierka\Zbiory\Zbior_DCT_mode5\Poprawne"
    niebo = 'D:\\STUDIA\\Cyberka\\Inzynierka\\Zbiory\\Niebo'
    niebo_dct = "D:\\STUDIA\\Cyberka\\Inzynierka\\Zbiory\\Niebo_DCT"
    fb_lsb = "D:\\STUDIA\Cyberka\Inzynierka\Zbiory\Facebook_lsb_v2"
    signal_lsb = "D:\STUDIA\Cyberka\Inzynierka\Zbiory\Signal_lsb"
    signal_lsb_zwiadomoscia = "D:\STUDIA\Cyberka\Inzynierka\Zbiory\Signal_lsb_v3_popobraniu_zwiadomoscia2"
    signal_lsb_odczytaj = "D:\STUDIA\Cyberka\Inzynierka\Zbiory\Signal_lsb_v3_odczytaj"
    facebook_dct = "D:\STUDIA\Cyberka\Inzynierka\Zbiory\W_czasie_inzynierki\Facebook"
    discord_dct = "D:\STUDIA\Cyberka\Inzynierka\Zbiory\W_czasie_inzynierki\Discord"
    steghide = "D:\STUDIA\Cyberka\Inzynierka\Zbiory\Steghide\Zbior"
    # processImagesInFolder(input_folder, output_folder)
    # canWeReadMessage(facebook_folder, worksheet)
    # changeNames(input_folder)
    # remove_alpha_channel(eight_folder, temp_folder)
    compareImages(input_folder, steghide, worksheet)
    # makeStegoImageDCT(discord_dct, discord_dct)
    # makeStegoImageLSB(signal_lsb, signal_lsb_zwiadomoscia)
    # canWeReadMessageLSB(signal_lsb_odczytaj, worksheet)
    # canWeReadMessageDCT(discord_dct, worksheet, 6335)
    # cutToEight(input_folder, eight_folder)
    workbook.close()
